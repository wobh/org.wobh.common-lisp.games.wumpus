...
;;;; Wounded Wumpuses and Hunters might heal.
...
;;;; The Wumpus might wake and move around at random before settling
;;;; down again.
...
(defpackage "ORG.WOBH.WUMPUS"
  ...
  (:use "COMMON-LISP")
  (:export "*HUNT-RANDOM-STATE*"
	   "HUNT-RANDOM" "HUNT-RANDOM-ELT" "HUNT-SHUFFLE")
  (:export "*ALLOW-QUIT*" "HUNT-END-P")
  (:export "*CAVE-NETWORKS*" "*DEFAULT-CAVE-NAME*" "GET-CAVE-NETWORK")
  (:export "*SETUP-WUMPUS-HEALTH*" "*WUMPUS-TRACKS-ARROW*")
  (:export "*SETUP-HUNTER-HEALTH*")
  (:export "*SETUP-QUIVER-ROOM*" "*SETUP-QUIVER-HOLD*" "*BOW-RANGE-MAXIMUM*")
  (:export "MAKE-HUNT" "SETUP-HUNT" "RESET-HUNT")
  (:export "MAKE-MESSAGE-TURN" "WRITE-WUMPUS-LINE")
  (:export "SETUP-GAME" "GAME-EVAL-HUNTER-ACTION")
  (:export "GAME-EVAL-HUNTER-SHOT" "GAME-EVAL-HUNTER-MOVE" "EVAL-PLAYER-QUIT" "READ-WUMPUS-CHAR")
  (:export "MAIN")
...
When true, a arrow-striken Wumpus, if not slain, will try to follow
the arrow into the room it came from.
...
(defparameter *default-cave-name* 'dodecahedron
...
(defstruct (hunt (:conc-name Nil)
  ...
  (cave-bats          ())
  (cave-pits          ())
  (wumpus-hurt        'wumpus-bothered)
  (wumpus-path        ())
  (wumpus-life        (list *setup-wumpus-health*))
  (hunter-path        ())
  (hunter-life        (list *setup-hunter-health*))
  (quiver-room        *setup-quiver-room*)
  (quiver-hold        (list *setup-quiver-hold*))
...
(defparameter *wumpus-tracks-arrow* Nil
  "Allow a wounded Wumpus to track the arrow down the tunnel it came
  out of.")

(defun setup-hunt (&key
		   (cave-name           *default-cave-name*)
		   (wumpus-health       *setup-wumpus-health*)
		   (wumpus-tracks-arrow *wumpus-tracks-arrow*)
		   (hunter-health       *setup-hunter-health*)
		   (bow-range-maximum   *bow-range-maximum*)
		   (quiver-room         *setup-quiver-room*)
		   (quiver-hold         *setup-quiver-hold*))
  (let* ((hunt
	  (make-hunt :cave-passages     (get-cave-network cave-name)
		     :wumpus-life       (list wumpus-health)
		     :wumpus-hurt       (if wumpus-tracks-arrow
					    'wumpus-enraged 'wumpus-bothered)
...
;; (defstruct (wumpus (:conc-name wumpus-))
;;   (life ())
;;   (path ()))
...
;; (defun setup-wumpus (health locale)
;;   (let ((wumpus (make-wumpus)))
;;     (push health (wumpus-life wumpus))
;;     (push locale (wumpus-path wumpus))
;;     wumpus))

;; (defun reset-wumpus (wumpus)
;;   (with-accessors ((life wumpus-life)
;; 		   (path wumpus-path)) wumpus
;;     (setf life (last life)
;; 	  path (last path))
;;     wumpus))

;;; TODO: I would prefer to have a wumpus-time slot which records
;;; wumpus events somewhat like so:

;; (defun wumpus-moment (wumpus)
;;   (first (wumpus-time wumpus)))

;; (defun tick-wumpus (moment wumpus)
;;   (push moment (wumpus-time wumpus)))


;;;; The Hunter

;; (defstruct (hunter (:conc-name hunter-))
;;   (life ())
;;   (path ()))
...
;; TODO: I sense a CLOS class being born

;; (defclass creature ()
;;   ((life
;;     :accessor creature-life
;;     :initarg :life)
;;    (path
;;     :accessor creature-path
;;     :initarg :path)))

;; (defmethod creature-health ((creature creature))
;;   (first (creature-life creature)))

;; (defmethod creature-alive-p ((creature creature))
;;   (< 0 (creature-health creature)))

;; (defmethod hurt-creature ((creature creature) &optional (hurt 1))
;;   (assert (< 0 hurt))
;;   (with-accessors ((life creature-life)
;; 		   (health creature-health)) creature
;;     (push (- health hurt) life)))

;; (defmethod heal-creature ((creature creature) &optional (heal 1))
;;   (assert (< 0 heal))
;;   (with-accessors ((life creature-life)
;; 		   (health creature-health)) creature
;;     (push (+ health heal) life)))

;; (defmethod creature-locale ((creature creature))
;;   (first (creature-path creature)))

;; (defun send-creature ((creature creature) locale)
;;   (assert (not (eq locale (wumpus-locale wumpus))))
;;   (push locale (creature-path creature)))

;; (defun setup-creature (health locale)
;;   (make-instance 'creature
;; 		 :life (list health)
;; 		 :path (list locale)))

;; (defun reset-creature (creature)
;;   (with-accessors ((life creature-life)
;; 		   (path creature-path)) creature
;;     (setf life (last life)
;; 	  path (last path))
;;     creature))
...
;; (defun setup-quiver (room hold)
;;   (let ((quiver (setup-quiver)))
;;     (setf (quiver-room quiver) room)
;;     (push (quiver-hold quiver) hold)
;;     quiver))
...
;; (defstruct (arrow (:conc-name arrow-))
;;   (path ()))

;; (defun send-arrow (locale arrow)
;;   (push locale (arrow-path arrow))

;; (defun reset-arrow (arrow)
;;   (setf (arrow-path arrow) ()))
...
(defun hunter-shoots (arrow-path hunt)
  ...
  (lose-arrow hunt)
  (eval-arrow-path-classic arrow-path hunt)
...
;; FIXME move-hunter creates an error if the move is invalid, use condition system.
...
;;;; FIXME: all these game options need better organization.


;;;; Wumpus Test package

;;;; (defpackage "ORG.WOBH.WUMPUS-TEST"
;;;;   (:nicknames "WUMPUS-TEST" "HUNT-TEST")
;;;;   (:use "COMMON-LISP" "ORG.WOBH.WUMPUS")
;;;;   (:export "*TEST-RANDOM-STATE*")
;;;;   (:export "*TEST-HUNT*" "SETUP-TEST-HUNT")
;;;;   (:export "TEST")
;;;;   (:documentation "Hunt the Wumpus test package"))

;;;; (in-package "WUMPUS-TEST")

;;;; (use-package "ORG.WOBH.WUMPUS")
...
(defun setup-test-hunt (&key
  ...
  (setf *test-hunt* (setup-hunt
		     :cave-name (get-cave-network cave-name)
		     :wumpus-health wumpus-health
		     :wumpus-tracks-arrow (if wumpus-tracks-arrow
					      'wumpus-rage 'wumpus-bothered)
		     :hunter-health hunter-health
		     :bow-range-maximum bow-range-maximum
		     :quiver-room   quiver-room
		     :quiver-hold   quiver-hold)))

;; (defun eval-hunter-action-test (env)
;;   "Shoot, move, or quit."
;;   (with-wumpus-input (input "Shoot, move, quit or show env "
;; 			    :readf #'read-wumpus-char)
;;     (case (char-downcase input)
;;       (#\s (funcall (symbol-function (game-eval-hunter-shot env))
;; 		    env))
...
(defun test (&optional hunt)
  (let ((*print-pretty* T)
	(env (setup-game :hunt (or hunt (setup-test-hunt))))
	(*allow-quit* T))
